#!/usr/bin/env bash
# waits for queue room, then submits the two hoptimus0 ERIN extraction arrays
DEP=$(squeue -u zuberi01 -h -n dl_hopt -o %i | head -1)
...
for try in $(seq 1 200); do
  ...
  for spec in "ea 0-1499%20 0" "eb 0-780%20 1500"; do set -- $spec
    ...
    if sbatch --job-name=$name --partition=cuda --gres=gpu:1 --array=$2 $DEP_OPT --time=00:30:00 --cpus-per-task=8 --mem=48G \
      ...
      --export=ALL,OFFSET=$3,ENCODER=hoptimus0 \
      --wrap="$SRC && python /mnt/scratche/slow/fmlab/zuberi01/phd/thesis/scripts/extract_one_hf.py --manifest /mnt/scratche/slow/fmlab/zuberi01/phd/thesis/campaigns/erin_manifest.txt --outdir /mnt/scratche/fast/fmlab/datasets/imaging/ERIN/features/20x_224px/features_hoptimus0"; then
