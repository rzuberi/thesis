...
T = "/mnt/scratche/slow/fmlab/zuberi01/phd/thesis"; OUT = os.environ.get("OUTDIR", "."); DEV = "cuda" if torch.cuda.is_available() else "cpu"; EPOCHS = int(os.environ.get("EPOCHS", "4")); TPS = int(os.environ.get("TPS", "2000"))
...
def train(tr, seed=0):
    ...
    return net.eval()
models = {}
for f in range(5): models[f] = train([k for k in keys if fold_of[k] != f]); print("fold model", f, flush=True)
models["final"] = train(keys); print("final model", flush=True); torch.save({k: v.state_dict() for k, v in models.items()}, os.path.join(OUT, "tile_mlp_models.pt"))
def summarise(path, net):
    with h5py.File(path) as h: X = np.asarray(h["features"], np.float32)
    with torch.no_grad(): P = torch.softmax(net(torch.tensor(X).to(DEV)), -1).cpu().numpy()
...
for i, p in enumerate(paths):
    u = os.path.basename(p).replace(".h5", ""); net = models[uuid_fold[u]] if u in uuid_fold else models["final"]
    try: v, P = summarise(p, net)
    except Exception as e: print("[skip]", p, e, flush=True); continue
    rows.append([p, u, "fold" if u in uuid_fold else "final"] + v.tolist())
...
res = {"n_summarised": int(len(df)), "n_fold_scored": int((df.model == "fold").sum()), "oof_1538_binary_LGDplus_auroc_mean_prob": round(float(roc_auc_score(yb, pb)), 4), "epochs": EPOCHS}
