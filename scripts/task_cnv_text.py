...
OUT = os.environ.get("OUTDIR", "."); MODEL = os.environ.get("MODEL", "medgemma:27b"); CONC = int(os.environ.get("CONC", "6"))
...
def describe(sid):
    ...
    return (f"Current biopsy histology: {g}. Shallow whole-genome sequencing relative copy number by chromosome arm "
...
out = os.path.join(OUT, f"cnv_text_{MODEL.replace(':', '_')}.csv"); done = set(pd.read_csv(out, dtype=str).sample_id) if os.path.exists(out) else set()
...
res = {"model": MODEL, "n_samples": len(d), "parsed": int(ok.sum()), "sample_auroc": round(float(roc_auc_score(d.y[ok], d.risk[ok])), 4), "patient_auroc_maxrisk": round(float(roc_auc_score(pat.y, pat.r)), 4),
