...
| Discussion 3: attention patches (item 7) | {'DONE' if 'intermediate_fusion' in M.get('item7', {}) else 'PARTIAL'} | {('Spearman image-only vs intermediate median ' + f3(M['item7']['intermediate_fusion']['overall']['spearman_median']) + ', vs co-attention ' + f3(M['item7']['coattention_fusion']['overall']['spearman_median']) + '; montages `results/paper_plan/figs/attention/`') if 'intermediate_fusion' in M.get('item7', {}) else 'attention rows pending'} |
...
**Method.** {i3['model']}. **Sources.** {SRC('main_items.json', 'pp_main.py')} (`item3`). **Caveats.** `BiopsyIndex`, `DaysSincePreviousBiopsy` and the row year encode surveillance history, not biology; 3b is a small subset with wide CIs.
""")
    # item 4
...
**Sources.** {SRC('main_items.json', 'pp_main.py')} (`item4_tables`). **Caveats.** The five fusion arms share the same image and CNV inputs; the selection-adjusted p treats the choice among them as post hoc. Subgroup tables are post hoc.
""")
    # operating point
