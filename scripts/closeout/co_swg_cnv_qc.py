...
sd_logratio, n_segments (runs of constant segmented value), frac_altered (|segmented| > 0.3). Row-level table stays on the
...
for cid, path in cx.drop_duplicates("cnv_id")[["cnv_id", "cnv_id"]].values:
    ...
    rows.append({"cnv_id": cid, "found": True, "n_bins": int(len(v)), "noise_mapd": float(np.median(diff)) if len(diff) else np.nan, "sd_logratio": float(np.std(v)), "n_segments": nseg, "frac_altered_0p3": float((np.abs(s) > 0.3).mean())})
