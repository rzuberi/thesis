...
- **Trainability:** node-positive (cN1–3: 96 vs 43 cN0; ypN1–3: 82 vs 51) and cT3 vs cT1–2 (111 vs 22) both clear the ~30-positive bar; full stage-group classification does not (most groups < 30).
- **RUN 21 Sep — nodal status from the pre-treatment OGD biopsy** (pre-registered in `docs/nodal_probe_preregistration.md`, same machinery as the response probe: OGD tiles only, ABMIL, patient-disjoint 5-fold × 3 seeds, 1,000-bootstrap CIs, 50-permutation null on UNI2). **Primary, pathological ypN+ vs ypN0 (n=133, 82 vs 51):** histology AUC by encoder — gigapath 0.604 [0.5031, 0.7027], hoptimus0 0.6341 [0.5357, 0.7325], phikon2 0.5784 [0.4685, 0.6832], uni_v2 0.6066 [0.5033, 0.7042], virchow2 0.6688 [0.5754, 0.7528]; clinical baseline *including the clinician's cN estimate* 0.6554 [0.5488, 0.759]; UNI2 permutation p = 0.0196 (null mean 0.4806, max 0.6062); tile-count confound AUC 0.5506. Pre-registered gate (lower CI > 0.5 and point ≥ 0.60): PASSED (marginally).
- **Secondary, clinical cN+ vs cN0 (n=139, 96 vs 43):** histology — gigapath 0.4981 [0.3976, 0.6122], hoptimus0 0.5388 [0.44, 0.6582], phikon2 0.5673 [0.4659, 0.6776], uni_v2 0.4867 [0.3831, 0.5993]; clinical baseline without the N column 0.672; permutation p = 0.6667 → **null**.
- **Reading (per the pre-registered interpretation rules):** the biopsy carries a *weak but real* signal about pathological node involvement (UNI2 0.61, permutation p 0.02; Virchow2 0.67, H-Optimus 0.63), yet it does **not** beat what the clinician already estimates from imaging (0.66 with cN) — so 'weak signal, not clinically useful on its own'. It carries no information about the radiologist's cN call (0.49): the histology signal, such as it is, tracks the pathologist's truth rather than the scan. Whether histology *adds* to the clinical model (a combined arm) was not pre-registered and is the natural next test; the Hannah Coles biopsies would roughly double the training set once linked.
